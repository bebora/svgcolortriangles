...
class Triangle:
    ...
    def add_light_source(self, source_direction, max_color_offset):
        vect_1 = [self.p2[0] - self.p1[0],
                  self.p2[1] - self.p1[1],
                  self.p2[2] - self.p1[2]]
        vect_2 = [self.p3[0] - self.p1[0],
                  self.p3[1] - self.p1[1],
                  self.p3[2] - self.p1[2]]
        
        triangle_vector = [
            vect_1[1] * vect_2[2] - vect_1[2] * vect_2[1],
            vect_1[2] * vect_2[0] - vect_1[0] * vect_2[2],
            vect_1[0] * vect_2[1] - vect_1[1] * vect_2[0]
        ]
        # triangle_vector = np.cross(self.p2 - self.p1, self.p3 - self.p1)
        # print(np.dot(triangle_vector, [0, 0, 1]))
        multiplier = triangle_vector[0] * source_direction[0] +\
                     triangle_vector[1] * source_direction[1] +\
                     triangle_vector[2] * source_direction[2]
        multiplier /= math.sqrt(source_direction[0] ** 2 +\
                                source_direction[1] ** 2 +\
                                source_direction[2] ** 2) *\
                      math.sqrt(triangle_vector[0] ** 2 +\
                                triangle_vector[1] ** 2 +\
                                triangle_vector[2] ** 2)
...
class SvgBox():
    ...
    def export(self):
        return self.head+self.get_body()+self.end
...
def main():
    try:
        ...
        print("Please provide a base filename")
        return
    ...
    for i in range(map.n_column, len(map.points)-1):
        if i % map.n_column != map.n_column-1:
            ...
            triangles_backup.append(t)
            # box.add_body(t.export_svg()+'\n')
    # Triangles generated without any color shading
    for i in range(0, ANIMATION_STEPS):
        angle = 2 * math.pi * i / ANIMATION_STEPS
        source_direction = [math.cos(angle), math.sin(angle), 0]
        new_box = copy.deepcopy(box)
        temp_triangles = copy.deepcopy(triangles_backup)
        for triangle in temp_triangles:
            triangle.add_light_source(source_direction, config.max_color_offset)
            new_box.add_body(triangle.export_svg()+'\n')
        with open(base_filename+str(i).zfill(4)+'.svg', 'w') as fp:
            fp.write(new_box.export())
    #box.export()
